...
function getSQLString() {
    return "
select mm.grp as grp, mm.nb as positif, mm1.nb as total, mm.client, mm1.channel, mm.daypart as DayPart,mm.campaign as campaign,dmin,dmax, mm.weekday as WEEKDAY ,
...
ORDER BY grp desc ) as mm , ( select AVG(`budgetnet` / ( `apres-count1` - `avant-count1` )) AS cpvi, sum(budgetnet),sum(`apres-count1`) , sum(`avant-count1`) ,client, count(*) as nb,Max(date) as dmax, MIN(date) as dmin, channel, ((sum(`apres-count1`) - sum(`avant-count1`))/(sum(grpref)*59093000)) as grp,
(
    CASE
WHEN (
dayofweek(date)=1
