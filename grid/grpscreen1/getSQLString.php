...
GROUP BY screen, WEEKDAY, channel, campaign order by grp ) as mm , ( select AVG(`budgetnet` / ( `apres-count1` - `avant-count1` )) AS cpvi, sum(budgetnet),sum(`apres-count1`) , sum(`avant-count1`) ,client, count(*) as nb, channel,Max(date) as dmax, MIN(date) as dmin,
(
    CASE
WHEN (
dayofweek(date)=1
